...
var InvoiceEditor = React.createClass({
  ...
  save: function () {
    if (this.state.enabled && '' === _.trim(this.state.invoiceCompany)) {
      layer.msg('要开发票就要填写发票抬头，公司类型的请填写公司名称，个人的请填写个人姓名');
    } else {
      ...
      <div className="row">
        <form method="post" className="qing-pad-tb-10 col-md-12 form-horizontal" id="addressEditForm">
          ...
          <div className="form-group clearfix">
            <label className="col-md-3 align-right control-label">{'开发票'}</label>
            <div className="col-md-9">
              <label className="radio-inline qing-radio">
                <input type="radio" name="isEnabled"  value="1" checkedLink={isEnabled.valueLink('1')}/>
                <i></i>{'是'}
              </label>

              <label  className="radio-inline qing-radio">
                <input type="radio" name="isEnabled" value="0" checkedLink={isEnabled.valueLink('0')}/>
                <i></i>{'否'}
              </label>
            </div>
          </div>
          <div className="form-group">
            <label className="col-md-3 align-right control-label">{'发票抬头'}</label>
            <div className="col-md-9">
          ...
          <div className="form-group align-center clearfix">
            <button className="btn " onClick={this.save} type="button">{'保存'}</button>
          &nbsp;
            <button className="btn " onClick={this.unSave} type="button">{'取消'}</button>
          </div>
        </form>
      </div>
    )
